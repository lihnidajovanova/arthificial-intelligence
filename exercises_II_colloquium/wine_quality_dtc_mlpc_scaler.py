...
def removed_col_split_set(set, col):
    new_set = [[row[i] for i in range(len(row)) if i != col] for row in set]
...
if __name__ == '__main__':
    ...
    dataset = [change(l1) for l1 in dataset]
    index_split = int(x * len(dataset))

    test_set = dataset[:index_split]
    train_set = dataset[index_split:]
    ...
    # print(list(classifierDT.feature_importances_))
    min_importance_feature = list(dt_clf.feature_importances_)
    index = min_importance_feature.index(min(min_importance_feature))

    train_x_1, train_Y_1 = removed_col_split_set(train_set, index)
    test_x_1, test_Y_1 = removed_col_split_set(test_set, index)

    mlp_clf = MLPClassifier(hidden_layer_sizes=15,
                            activation='relu',
                            learning_rate_init=0.001,
                            max_iter=200,
                            random_state=0)
    scalerSS = StandardScaler()
    scalerSS.fit(train_x_1)
    scalerMMS = MinMaxScaler()
    scalerMMS.fit(train_x_1)

    mlp_clf.fit(scalerSS.transform(train_x_1), train_Y_1)
    accuracy_1 = accuracy_calculation(mlp_clf.predict(scalerSS.transform(test_x_1)), test_Y_1)
    print(f"Tocnost so StandardScaler: {accuracy_1}")

    mlp_clf.fit(scalerMMS.transform(train_x_1), train_Y_1)
    accuracy_2 = accuracy_calculation(mlp_clf.predict(scalerMMS.transform(test_x_1)), test_Y_1)
    print(f"Tocnost so MinMaxScaler: {accuracy_2}")
