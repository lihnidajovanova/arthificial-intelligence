...
def removed_column_set(set, idx):
    return [[row[i] for i in range(0, len(row)) if i != idx] for row in set]
...
if __name__ == '__main__':
    ...
    column = int(input())
    example = list(map(float, input().split(" ")))

    train_set = dataset[:idx(dataset, 0.8)]
    val_set = dataset[idx(dataset, 0.8):]

    train_x, train_y = split_xy(train_set)
    val_x, val_y = split_xy(val_set)

    scaler = MinMaxScaler(feature_range=(-1, 1))
    scaler.fit(train_x)

    clf = MLPClassifier(random_state=0, max_iter=20, activation="relu", hidden_layer_sizes=neurons,
                        learning_rate_init=learn_rate)
    clf.fit(scaler.transform(train_x), train_y)

    train_acc = accuracy(train_y, clf.predict(scaler.transform(train_x)))
    val_acc = accuracy(val_y, clf.predict(scaler.transform(val_x)))

    if train_acc > val_acc * 1.15:
        new_train_x = removed_column_set(train_x, column)
        scaler.fit(new_train_x)
        clf.fit(scaler.transform(new_train_x), train_y)
        example = [example[i] for i in range(0, len(example)) if i != column]
        print("Se sluchuva overfitting")
    else:
        print("Ne se sluchuva overfitting")

    print(clf.predict(scaler.transform([example]))[0])
