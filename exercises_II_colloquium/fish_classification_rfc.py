...
if __name__ == "__main__":
    ...
    col_index = int(input())  # column index to delete
    num_of_trees = int(input())  # number of trees in the forest
    criterion = input()  # splitting criterion of the tree
    new_list = [el for el in input().split(" ")]  # characteristics of the fish to classify

    # delete the specified column from the dataset
    del new_list[col_index]

    # divide the dataset into train sets
    train_set = dataset[:int(0.85 * len(dataset))]
    train_x = [t[:-1] for t in train_set]
    train_y = [t[-1] for t in train_set]

    # divide the dataset into test sets
    test_set = dataset[int(0.85 * len(dataset)):]
    test_x = [t[:-1] for t in test_set]
    test_y = [t[-1] for t in test_set]

    # remove the element at the given column index from the training set
    train_x_1 = list()
    for t in train_x:
        sample = [t[i] for i in range(len(t)) if i != col_index]
        train_x_1.append(sample)

    # remove the element at the given column index from the testing set
    test_x_1 = list()
    for t in test_x:
        sample = [t[i] for i in range(len(t)) if i != col_index]
        test_x_1.append(sample)

    # initialize decision tree classifier (random forest) and train it
    rf_clf = RandomForestClassifier(n_estimators=num_of_trees, criterion=criterion, random_state=0)
    rf_clf.fit(train_x_1, train_y)

    # initialize a counter for the number of correct predictions
    accuracy = 0
    for x, y in zip(test_x_1, test_y):
    ...
    fish_characteristics = rf_clf.predict([new_list])[0]
    print(fish_characteristics)

    # get the probability of each class for the input data
    probability = rf_clf.predict_proba([new_list])[0]
    print(probability)
