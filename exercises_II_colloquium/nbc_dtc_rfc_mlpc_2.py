...
def generate_classifier(clf):
    ...
    if clf == 'NB':
        ...
        classifier = MLPClassifier(hidden_layer_sizes=10, activation="relu", learning_rate_init=0.001, random_state=0)

    return classifier


def index(set, percent):
    return int(len(set) * percent)
...
def split_balanced(dataset, percent):
    set_0 = [row for row in dataset if row[-1] == 0]
    set_1 = [row for row in dataset if row[-1] == 1]

    train_set = set_0[:index(set_0, percent)] + set_1[:index(set_1, percent)]
    test_set = set_0[index(set_0, percent):] + set_1[index(set_1, percent):]

    return train_set, test_set


def key_with_max_value(dict: dict):
    max = -1
...
if __name__ == '__main__':
    ...
    train_set, test_set = split_balanced(dataset, x)
    train_x, train_y = split_set(train_set)
    test_x, test_y = split_set(test_set)

    weights = {"NB": 1, "DT": 1, "RF": 1, "MLP": 1}
    clf_names = {"NB": "Naive Bayes", "DT": "Decision Tree", "RF": "Random Forest", "MLP": "MLP"}
    classifiers = {}
    ...
    for c in weights.keys():
        ...
        current_accuracy = accuracy_score(test_y, classifier.predict(test_x))
        if current_accuracy > max_accuracy:
            max_accuracy = current_accuracy
            max_classifier = c

    print(f"Najgolema tocnost ima klasifikatorot {clf_names[max_classifier]}")
    weights[max_classifier] += 1
    ...
    for x, y in zip(test_x, test_y):
        ...
        for c in weights.keys():
            ...
            if votes.__contains__(prediction):
                votes[prediction] += weights[c]
            else:
                votes[prediction] = weights[c]
        max_vote = key_with_max_value(votes)
        predictions.append(max_vote)
