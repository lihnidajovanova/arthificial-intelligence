...
def split_subset(set):
    set_x = [row[0:-1] for row in set]
    set_y = [row[-1] for row in set]

    return set_x, set_y


def create_train_clf(train_x, train_y):
...
if __name__ == '__main__':
    ...
    # initialize classifiers and train them
    dt_clf_1 = create_train_clf(first_train_x, first_train_y)
    dt_clf_2 = create_train_clf(second_train_x, second_train_y)
    dt_clf_3 = create_train_clf(third_train_x, third_train_y)
    ...
    votes = {}
    for (i, c) in enumerate([dt_clf_1, dt_clf_2, dt_clf_3]):
        votes[i] = c.predict([test_case[0:-1]])[0]

    # print the results
    print(f"Glasovi: {count_votes(votes)}")
    print(f"Predvidena klasa: {calc_dominate_vote(votes)}")
