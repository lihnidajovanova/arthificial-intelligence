...
class Disk(Problem):
    ...
    def successor(self, state):
        ...
        for d in range(len(array)):
            if array[d] != "-":
                if d + 1 < len(array) and array[d + 1] == "-":
                    new_field = list(array)
                    new_field[d] = "-"
                    new_field[d + 1] = array[d]
                    successors[f"D1: Disk {array[d]}"] = tuple(new_field)
                if d + 2 < len(array) and array[d + 1] != "-" and array[d + 2] == "-":
                    new_field = list(array)
                    new_field[d] = "-"
                    new_field[d + 2] = array[d]
                    successors[f"D2: Disk {array[d]}"] = tuple(new_field)
                if d > 0 and array[d - 1] == "-":
                    new_field = list(array)
                    new_field[d] = "-"
                    new_field[d - 1] = array[d]
                    successors[f"L1: Disk {array[d]}"] = tuple(new_field)
                if d - 1 > 0 and array[d - 1] != "-" and array[d - 2] == "-":
                    new_field = list(array)
                    new_field[d] = "-"
                    new_field[d - 2] = array[d]
                    successors[f"L2: Disk {array[d]}"] = tuple(new_field)
        return successors
...
if __name__ == "__main__":
    ...
    for i in range(n):
        goal_initial[i] = disks[i] + 1
        disk_result[i] = disks[i] + 1

    # reversed initial state as a goal check
    goal_initial.reverse()

    disk = Disk(tuple(disk_result), tuple(goal_initial))

    answer = breadth_first_graph_search(disk)
