...
class Squares(Problem):
    ...
    def successor(self, state):
        ...
        for square in range(5):
            x, y = state[square]
            ordered_list = list(state)

            ordered_list[square] = (x, y + 1)
            if self.check_valid(ordered_list):
                succ[f'Pomesti kvadratche {square + 1} gore'] = tuple(ordered_list)

            ordered_list[square] = (x, y - 1)
            if self.check_valid(ordered_list):
                succ[f'Pomesti kvadratche {square + 1} dolu'] = tuple(ordered_list)

            ordered_list[square] = (x - 1, y)
            if self.check_valid(ordered_list):
                succ[f'Pomesti kvadratche {square + 1} levo'] = tuple(ordered_list)

            ordered_list[square] = (x + 1, y)
            if self.check_valid(ordered_list):
                succ[f'Pomesti kvadratche {square + 1} desno'] = tuple(ordered_list)
