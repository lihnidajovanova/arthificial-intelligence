    ...
    def successor(self, state):
        ...
        for i in range(self.n):
            for j in range(self.n):
                rez = self.generate_state(state, i, j)
                successors["x: {}, y: {}".format(i, j)] = rez
        return successors

    def generate_state(self, state, i, j):
        matrix = list(map(list, state[0]))
        count = 0
        matrix[i][j] = abs(matrix[i][j] - 1)
        if i > 0:
            matrix[i - 1][j] = abs(matrix[i - 1][j] - 1)
        if i < self.n - 1:
            matrix[i + 1][j] = abs(matrix[i + 1][j] - 1)
        if j > 0:
            matrix[i][j - 1] = abs(matrix[i][j - 1] - 1)
        if j < self.n - 1:
            matrix[i][j + 1] = abs(matrix[i][j + 1] - 1)

        for _ in range(self.n):
            count += matrix[_].count(0)

        next_state = (tuple(map(tuple, matrix)), count)
        return next_state
...
if __name__ == "__main__":
    ...
    for i in range(n):
        row = row_input[i * n: (i + 1) * n]
        initial_matrix.append(row)
        count_ones += row.count(0)
    initial_matrix = tuple(map(tuple, initial_matrix))
    initial_ = (initial_matrix, count_ones)
    problem = Matrix(initial_, n)

    game = breadth_first_graph_search(problem)
    if game is not None:
        print(game.solution())
    else:
