    ...
    def successor(self, state):
        ...
        for i in range(len(state)):
            ...
            for action, direction in zip(actions, directions):
                rez = self.move(state, i, direction)
                if rez is not None:
                    succ[action] = rez
        return succ

    def move(self, state, i, direction):
        squares = list(state)
        move_square = list(squares[i])
        move_square[0] += direction[0]
        move_square[1] += direction[1]
        move_square = tuple(move_square)
        squares[i] = move_square
        squares = tuple(squares)
        if self.check_valid(squares):
            return squares
        else:
            return None
...
if __name__ == '__main__':
    ...
    game = breadth_first_graph_search(squares)
    if game is None:
