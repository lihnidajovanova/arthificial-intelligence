...
class Pacman(Problem):
    ...
    def h(self, node):
        # maksimalniot mhd za da se stigne do krajnata tocka
        x_state = node.state[0]
        y_state = node.state[1]
        lista_tocki = node.state[3]

        list_mhd = list()

        if len(lista_tocki) > 0:
            for tocka in lista_tocki:
                mhd = abs(x_state - tocka[0]) + abs(y_state - tocka[1])
                list_mhd.append(mhd)
        else:
            return 0
