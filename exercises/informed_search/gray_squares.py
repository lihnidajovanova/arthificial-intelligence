...
class Squares(Problem):
    ...
    """
    Хевристичката функција која е дадена во кодот е базирана на Manhattan distance помеѓу 
    тековната состојба на проблемот и целната состојба. 
    Манхетн расстојанието е збирот на апсолутните разлики во редиците и колоните помеѓу две точки.
    Давајќи кратко објаснување за кодот:
    -node претставува тековниот јазол во пребарувачкото дрво, а node.state е тековната состојба на проблемот.
    -self.goal е целната состојба кон која сакаме да стигнеме.
    """

    def h(self, node):
        mhd = [abs(state[0] - goal[0]) + abs(state[1] - goal[1]) for state, goal in zip(node.state, self.goal)]
        return max(mhd)
