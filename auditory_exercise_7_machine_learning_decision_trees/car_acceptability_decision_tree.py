...
if __name__ == "__main__":
    ...
    train_sat = dataset[:int(0.7 * len(dataset))]
    train_x = [row[:-1] for row in train_sat]
    train_y = [row[-1] for row in train_sat]
    ...
    test_x_3 = list()
    for t in test_x:
        row = [t[i] for range(len(t)) if i != least_important_feature]
        test_x_3.append(row)
