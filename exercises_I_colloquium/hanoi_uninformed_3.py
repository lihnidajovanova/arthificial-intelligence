...
class DonutGame(Problem):
    ...
    def successor(self, state):
        ...
        for i in range(0, n):
            for j in range(0, n):
                stolbovi = [list(el) for el in state]

                if i == j or stolbovi[i][0] == 1000:
                    continue
                # print(stolbovi)
                if stolbovi[j][-1] >= stolbovi[i][-1]:

                    donut_to_transfer = stolbovi[i].pop()
                    if len(stolbovi[i]) == 0:
                        stolbovi[i].append(1000)
                    stolbovi[j].append(donut_to_transfer)
                    stolbovi[j] = [donut for donut in stolbovi[j] if donut != 1000]

                    stolbovi = [tuple(d) for d in stolbovi]
    ...
    def goal_test(self, state):
        return list(state) == list(self.goal)
...
if __name__ == "__main__":
    ...
    n = len(i1.split(";"))
    lists_initial = i1.split(";")
    lists_goal = i2.split(";")

    initial_state = []

    for l in lists_initial:
        tmp_l = l.split(",")
    ...
    # print(initial_state)
    # print(goal_state)
    # print(goal_state == ((0,), (0,), (3, 2, 1)))
    # print(list(initial_state[-1]))
    donut_game = DonutGame(initial_state, goal_state)
    result = breadth_first_graph_search(donut_game)
