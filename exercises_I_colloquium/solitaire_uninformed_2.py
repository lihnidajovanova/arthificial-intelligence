...
class BallGame(Problem):
    ...
    def __init__(self, initial, size_matrix, walls):
        super().__init__(initial)
        self.size_matrix = size_matrix
        self.walls = walls
        self.goal = (size_matrix // 2, size_matrix - 1)
    ...
    def successor(self, state):
        ...
        for i in range(0, len(balls)):
            for move, coord, between in zip(moves, moves_coord, in_between):
                ...
                if self.in_range(new_ball_coords):
                    coord_in_between = [local_balls[i][0] + between[0], local_balls[i][1] + between[1]]
                    if coord_in_between in local_balls:
                        old_x, old_y = local_balls[i]
                        local_balls[i] = new_ball_coords
                        local_balls = [tuple(b) for b in local_balls if b != coord_in_between]
    ...
    def in_range(self, coord):
        ...
        if coord in self.walls:
            return False
        return 0 <= x < self.size_matrix and 0 <= y < self.size_matrix
...
if __name__ == "__main__":
    ...
    for i in range(0, num_balls):
        ball_coords.append(tuple([int(a) for a in input().split(",")]))
    num_walls = int(input())
    wall_coords = []
    for i in range(0, num_walls):
        wall_coords.append(tuple([int(a) for a in input().split(",")]))

    # print(n,ball_coords,wall_coords)
    ball_coords = tuple(ball_coords)
    wall_coords = tuple(wall_coords)

    initial_state = ball_coords

    ballGame = BallGame(initial_state, n, wall_coords)
    result = breadth_first_graph_search(ballGame)
