...
# def up_1(x, y, walls, n):
#     new_x = x
#     new_y = y + 1
#     if GhostOnSkates.check_valid((new_x, new_y), walls, n):
#         return new_x, new_y
#     return x, y


# def up_2(x, y, walls, n):
#     new_x = x
#     new_y = y + 2
#     if GhostOnSkates.check_valid((new_x, new_y), walls, n):
#         return new_x, new_y
...
class GhostOnSkates(Problem):
    ...
    def check_valid(state, walls, n):
        x = state[0]
        y = state[1]
        if x < 0 or y < 0 or x >= n or y >= n or (x, y) in walls:
            return False
        return True
    ...
    def successor(self, state):
        ...
        if self.check_valid((x, y + 1), walls, n):
            new_x = x
            new_y = y + 1
            successors["Gore 1"] = new_x, new_y

        if self.check_valid((x, y + 2), walls, n):
            new_x = x
            new_y = y + 2
            successors["Gore 2"] = new_x, new_y

        if self.check_valid((x, y + 3), walls, n):
            new_x = x
            new_y = y + 3
            successors["Gore 3"] = new_x, new_y

        if self.check_valid((x + 1, y), walls, n):
            new_x = x + 1
            new_y = y
            successors["Desno 1"] = new_x, new_y

        if self.check_valid((x + 2, y), walls, n):
            new_x = x + 2
            new_y = y
            successors["Desno 2"] = new_x, new_y

        if self.check_valid((x + 3, y), walls, n):
            new_x = x + 3
            new_y = y
            successors["Desno 3"] = new_x, new_y
    ...
    def h(self, node):
        ...
        mhd = abs(x_ghost - x_pacman) + abs(y_ghost - y_pacman)
        return mhd / 3
